...
--Height: terrain_height + mountain_height - water_noise - 450
...
function GetColorAt( x, y, lod )
	...
	if isInWaterHeight(l_height) then
		local _noise = _sm_noise_octaveNoise2d(x, y, 7, g_terrainSeed) * 0.2 + 0.6
		return _noise, _noise, 0.1
	end
	...
	local _noise = _sm_noise_octaveNoise2d(x, y, 6, g_terrainSeed) * 0.25 + 0.9
	return 0, _util_clamp(_noise, 0, 1), 0
end
...
local _water_position = sm.vec3.new(32, 32, -480)
local _water_scale = sm.vec3.new(64, 20, 64)
